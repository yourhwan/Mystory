                    ...
                    if (answer11 == 1) { // 집밥
                            ...
                            if (answer47 == 1) { // 유튜브 보기
                                ...
                                if (answer96 == 1) { // 소맥
                                    ...
                                    if (answer98 == 1) { // 얌전히 돌아간다
                                        System.out.println("얌전히 집에 돌아가기 위해 주섬주섬 옷을 챙겨입고 인사를 하는 백수 오윤환, 그 녀석들이 도발을 한다." +
                                                "\n그렇다, 백수 오윤환은 그들이 도발 해주길 내심 기다렸다. 다시 착석해 미친둣이 술을 마신다.");
                                        System.out.println("\n그렇게 백수 오윤환과 그의 친구들은 그대로 쓰러져 잠들어버린다... 나름 알찬 하루를 보내려고 노력했던 백수 오윤환," +
                                                "\n착실한 하루를 보내지는 못했지만 오랜만에 만난 친구들과 우정을 나누며 즐거운 시간을 보냈기에 이 또한 나름대로 의미가 있던 시간이 아닐까...?" +
                                                "\n-THE END- ");
                                        sc.close();
                                    }
                                        ...
                                        if (answer104 == 1) { // 도발에 넘어간다
                                            System.out.println("백수 오윤환의 광역 도발에 친구들이 분노를 하며 술이 가득 든 잔을 원샷한다. 도발을 했지만 예상과는 다른 반응에 공포가 엄습하는 백수 오윤환.");
                                            System.out.println("\nQ.백수 오윤환의 광역 도발로 인해 과열이 되어버린 그들은 다시 술잔을 기울인다. 안주가 모자른 지금, 집주인 친구가 씨익 웃으며 안주를 가지러 주방으로 향한다. 어떤 안주를 더 먹을까?" +
                                                    "\n1.과자    2.라면    3.먹지 않는다");
                                            ...
                                            if (answer105 == 1) { // 과자
                                                System.out.println("어디선가 과자를 가져오는 친구. 아무래도 술자리가 끝나려면 아직 멀었나보다. 엄슴한 공포에 휩싸인 백수 오윤한.");
                                            ...
                                            System.out.println("광역 도발에도 평정심을 유지하는 친구들, 백수 오윤환은 내심 안심한 듯 하다. 사실 한 잔만 더 마시면 위험할 것 같았던 백수 오윤환이다.");

                                        }
                                        else { // 패배를 인정한다
                                            System.out.println("백수 오윤환의 광역 도발에 소스라치게 놀라며 패배를 인정하는 친구들, 더 이상은 마시지 못하겠다며 살려달라고 애원한다." +
                                                    "\n승리의 기쁨에 취한 백수 오윤환은 세레모니로 혼자 한 잔을 더 마시더니 그대로 기절해 잠들어버린다..." +
                                                    "\n-THE END-");
                                            sc.close();
                                        }
                                    }
                        ...
                        if (answer22 == 1) { // 너구리
                            ...
                            if (answer48 == 1) { // LP감상
                                ...
                                System.out.println("\nQ.구매했던 모든 LP를 듣다보니 시간이 훌쩍 흘렀다. 이때 엄마가 아빠와 연애시절 선물 받았던 LP가 집에 있다는게 생각난 백수 오윤환," +
                                        "\n그 LP들도 전부 들어볼까? " +
                                        "\n1.들어보자    2.이제 그만 듣자");
                                ...
                                if (answer99 == 1) { // 들어보자
                                    System.out.println("찾아보니 유재하, 소방차, 이문세 등 그 시절 유명했던 가수들의 LP가 상당히 많다. 잔뜩 신이 나버린 백수 오윤환, 이후 해야할 것들은 잊은채 다시 감상모드에 돌입한다.");
                                    System.out.println("\nQ.홀린듯 노래를 들으며 갖고싶은 LP들까지 디깅 하다보니 시간이 한참 흘러 밖은 어두워졌다... 시간을 확인해보는 백수 오윤환 현재 시간은 오후 6시가 훌쩍 넘었다." +
                                            "\n이미 무언가 하기는 글렀다고 판단한 백수 오윤환은 오늘은 쉬기로 한다. 무엇을 하며 쉴까?" +
                                            "\n1.가족들과 시간 보내기    2.해외축구 보기");
                                    ...
                                    if (answer100 == 1) { // 가족들과 시간 보내기
                                        System.out.println("가족들과 시간을 보내기로한 백수 오윤환, 거실로 나가 오랜만에 함께 TV도 보고 도란도란 이야기도 나누며 시간을 보낸다.");
                                        System.out.println("가족들과 시간을 보내다보니 어느덧 저녁 11시가 되었다. 비록 생산적인 것들은 하지 못했지만, 오랜만에 여가생활도 즐기고" +
                                                "\n소소하지만 정말 오랜만에 온 가족이 모여 함께 시간을 보낼 수 있어서 나름 괜찮았던 하루였던 것 같다. 백수 오윤환은 내일 더 나은 하루를 보내자고 다짐하며 이불 속에 들어가 잠을 청한다." +
                                                "\n-THE END-");
                                        sc.close();
                                    }
                                    else { // 해외축구 보기
                                        System.out.println("오늘은 헤외축구 경기가 많이 있다. 오랜만에 해외축구를 챙겨볼 생각에 신이 난 백수 오윤환, 컴퓨터를 켜고 해외축구를 보기 시작한다.");
                                        System.out.println("연달아 경기를 보다 그대로 잠에 든 백수 오윤환, 지난 1년이 넘는 시간 동안 이런저런 사정들로 제대로 된 여가생활을 즐기지 못했지만 오늘은 좋아하는 음악도 실컷 듣고, 좋아하는 해외축구도 실컷 보았다." +
                                                "\n소소하지만 이런 작은 것들이 어쩌면 쌓이고 쌓여 내일을 살아가는 원동력 중 하나가 되지 않을까? 내일의 백수 오윤환은 오늘을 원동력 삼아 자신의 발전에 투자하는 하루를 살 수 있을 것 같다." +
                                                "\n-THE END-");
                                        sc.close();
                                    }
                                }
                                else { // 그만 듣자
                                    System.out.println("이미 LP를 듣느라 시간이 너무나 많이 흘렀다. 공부를 해야겠다고 생각을 한 백수 오윤환, 컴퓨터 앞에 앉아 공부를 시작한다.");
                                    System.out.println("\nQ.개발 공부를 시작하는 백수 오윤환, 오늘은 인프런에서 구매했던 강의를 들으려고 한다. 오늘의 공부 목표를 정하려는데 몇강까지 공부를 할까?" +
                                            "\n1.3강    2.4강");
                                    ...
                                    if (answer101 == 1) { // 3강
                                        System.out.println("강의 3강까지 공부를 하기로 결심한 백수 오윤환, 시간이 얼마가 걸리든 목표한 강의까지 공부하기 위해 집중해서 공부를 하기 시작한다..");
                                        System.out.println("\nQ.목표한 3강까지 집중해서 꼼꼼히 공부를 마친 백수 오윤환, 현재 시간은 오후 9시가 되었다. 공부는 마치고 다른 것을 하려고 하는데 무엇을 할까?" +
